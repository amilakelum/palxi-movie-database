...
export default async function Hero() {
  try {
    ...
    const browseLink =
      backdrop.media_type === "movie" ? "/movies/trending" : "/tv/trending";
    ...
    return (
      <div className="relative rounded-xl overflow-hidden aspect-[21/9]">
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-8">
          <h1 className="text-2xl md:text-4xl font-bold mb-2 max-w-2xl">
            {backdrop.title || backdrop.name}
          </h1>
          <p className="overview text-sm md:text-base text-muted-foreground mb-4 max-w-2xl line-clamp-2 md:line-clamp-3">
            {backdrop.overview}
          </p>
          <div className="flex gap-3">
            ...
            <Button variant="outline" asChild>
              <Link href={browseLink}>
                Browse {backdrop.media_type === "movie" ? "Movies" : "TV Shows"}
              </Link>
            </Button>
          </div>
        </div>
    ...
    return (
      <div className="relative rounded-xl overflow-hidden aspect-[21/9] bg-muted">
        <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-8">
          ...
          <p className="overview text-sm md:text-base text-muted-foreground mb-4 max-w-2xl">
